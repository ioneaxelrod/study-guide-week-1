...
def duplicates(items):
    ...
    word_occurances = {}
    for item in items:
        if item not in word_occurances:
            word_occurances[item] = 1
        else:
            word_occurances[item] += 1
    dupe_list = []
    for key in word_occurances:
        if word_occurances[key] > 1:
            dupe_list.append(key)
    return sorted(dupe_list)
